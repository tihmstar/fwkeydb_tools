...
def productForCPIDAndBDID(cpid,bdid):
  for d in irecv_devices:
    if d[2] == bdid and d[3] == cpid:
      return d[0]
  raise Exception("Failed to find Product for cpid '0x%x' and bdid '0x%x"%(cpid,bdid))

def bdidForProductAndCPID(product, cpid):
  for d in irecv_devices:
